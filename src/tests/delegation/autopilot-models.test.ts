...
test("runAutopilot resolves per-job reasoning-effort overrides from env", async () => {
  await withTmpDir("codex-specialized-subagents-autopilot-reasoning-effort-", async (tmpDir) => {
    const codexHome = path.join(tmpDir, "codex_home");
    const env = {
      ...process.env,
      CODEX_HOME: codexHome,
    ...
    assert.equal(scan?.thinking_level, "low");
    assert.deepEqual(scan?.config_overrides, ['model_reasoning_effort="low"']);

    assert.equal(implement?.thinking_level, "high");
    assert.deepEqual(implement?.config_overrides, ['model_reasoning_effort="xhigh"']);

    assert.equal(verify?.thinking_level, "low");
    assert.deepEqual(verify?.config_overrides, ['model_reasoning_effort="low"']);
    ...
    assert.deepEqual(scanCall?.configOverrides, ['model_reasoning_effort="low"']);
    assert.deepEqual(implementCall?.configOverrides, ['model_reasoning_effort="xhigh"']);
    assert.deepEqual(verifyCall?.configOverrides, ['model_reasoning_effort="low"']);
